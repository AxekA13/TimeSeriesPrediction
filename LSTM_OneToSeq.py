...
lookback = 10  # на сколько шагов назад смотрим при предсказании
...
model = Sequential()
model.add(
    LSTM(50, return_sequences=True, input_shape=(lookback, feature), activation='linear'))
...
title = 'LSTM_1to1 2 layers, 50 neurons. 10 epochs.Act=linear, look=10,pred=1'
...
rmse_show = 'Test RMSE: %.3f' % rmse
print('Test RMSE: %.3f' % rmse)
plt.xlabel(rmse_show)

# Все предсказания и настоящее значение
